import os, sys, webbrowser, pathlib, clipboard, psutil, subprocess, time

current_directory = pathlib.Path(__file__).parent.parent.absolute()
...
class App():
	...
	def load_file(self, file_name):
        # TODO: if file does not exist create it and exit function
		# print(os.path.exists('.ignore'))

		if not os.path.exists('.ignore'):
			self.save_file('.ignore', '\n')
			return
	...
	def ignore(self, app_to_ignore):
		...
		if application_name == '':
			...
			if application_name in self.running_apps():
				...
				if not os.path.exists('.ignore'):
					self.setup_files(self.session_name)

				if os.path.exists(self.session_name + '-software.ses'):
	...
	def setup_files(self, session_name):
		self.save_file(session_name + '-software.ses', '')
		self.save_file(session_name + '-browser.ses', '')
		self.save_file('.ignore', '')
	...
	def show_all_running_apps(self, placeholder=None):
		...
		for app in self.running_apps():
			if not apps_to_be_ignored == None:
	...
	def list_active_sessions(self, placeholder=None):
		if len(self.get_active_sessions()) == 0:
